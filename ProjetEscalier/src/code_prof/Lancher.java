...
public class Lancher {
	...
	public static BufferedImage getFloatBuuffredImage(int w, int h) {
		...
		SampleModel sampleModel = new PixelInterleavedSampleModel(DataBuffer.TYPE_FLOAT, w, h, bands, w * bands,
				bandOffsets);
		...
		ColorModel colorModel = new ComponentColorModel(colorSpace, false, false, Transparency.TRANSLUCENT,
	...
	public static BufferedImage convolve(BufferedImage img, int[][] mask, int mask_size) {
		...
		for (int i = 1; i < rows - 1; i++) {
			for (int j = 1; j < cols - 1; j++) {
				...
				// System.out.println("\tseted val => " + "a = " + a + " r = " + r + " g = " + g
				// + " b = " + b);
