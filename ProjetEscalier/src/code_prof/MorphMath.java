...
public class MorphMath {
	...
	/**
	 * Fonction blur : Définition floue de l'élément structurant des opérations de
	 * dilatation et d'érosion
	 * 
	 * @param t la distance entre le pixel en cours de traitement (centre de
	 *          l'élément structurant) et du point de l'élément structurant courant
	...
	/**
	 * Fonction hamming : Calcul de la norme Hamming entre le pixel en cours de
	 * traitement et le point de l'élément structurant courant
	 * 
	...
	/**
	 * Fonction convertToGrayscale : Conversion d'un objet BufferedImage en niveau
	 * de gris
	 * 
	 * @param source la BufferedImage correspondant à l'image que l'utilisateur a
	 *               choisi
	 ...
	 * 
	 * @param img la BufferedImage source, qui ne sera pas modifié à l'intérieur de
	 *            la fonction
	...
	public static BufferedImage erode(BufferedImage img, int a) {
		// On crée la BufferedImage destination, de même dimension que la source, et en
		// niveau de gris
		...
		for (int i = 0; i < img.getHeight(); i++) {
			for (int j = 0; j < img.getWidth(); j++) {
				...
				newRGB = (0xFF << 24) | ((newGrayLevel & 0xFF) << 16) | ((newGrayLevel & 0xFF) << 8)
						| ((newGrayLevel & 0xFF));
	...
	/**
	 * Fonction mask_erode : Calcul du niveau de gris à affecter au pixel courant à
	 * partir des niveaux de gris des voisins
	 * 
	...
	private static int mask_erode(int x, int y, BufferedImage img, int a) {
		// Variables temporaires stockant les deux valeurs calculées de niveau de gris,
		// pour chaque pixel de l'élément structurant
		double tempMuGL, tempFGL;
		// Variable temporaire stockant le maximum des deux variables temporaires
		// ci-dessus
		int maxGL;
		...
		for (int i = -a; i <= a; i++) {
			for (int j = -a; j <= a; j++) {
				...
				if (!imgEdge) {
					// Récupération de la valeur par la fonction d'appartenance de l'ensemble flou
					// de l'image
					...
					// Et on stocke finalement la valeur calculée si elle est plus faible que la
					// valeur récupérée dans les tours de boucles précédents
					if (maxGL < finalGL) {
						finalGL = maxGL;
					}
				}
	...
	/**
	 * Fonction mask_dilate : Calcul du niveau de gris à affecter au pixel courant à
	 * partir des niveaux de gris des voisins
	 * 
	 * @param x   l'abscisse du pixel courant
	 * @param y   l'ordonnée du pixel courant
	...
	private static int mask_dilate(int x, int y, BufferedImage img, int a) {
		...
		// pour chaque pixel de l'élément structurant
		double tempMuGL, tempFGL;
		// Variable temporaire stockant le minimum des deux variables temporaires
		// ci-dessus
		int minGL;
		...
		for (int i = -a; i <= a; i++) {
			for (int j = -a; j <= a; j++) {
				...
				if (!imgEdge) {
					...
					// Et on stocke finalement la valeur calculée si elle est plus forte que la
					// valeur récupérée dans les tours de boucles précédents
					if (minGL > finalGL) {
						finalGL = minGL;
					}
	...
	/**
	 * Fonction open : Ouverture mathématique de l'image, soit une érosion, puis une
	 * dilatation successive de l'image
	 * 
	...
	public static BufferedImage open(BufferedImage img, int a) {
		// On définie une BufferedImage vide, aux dimensions de la BufferedImage source,
	...
	/**
	 * Fonction close : Fermeture mathématique de l'image, soit une dilatation, puis
	 * une érosion successive de l'image
	 * 
	 * @param img la BufferedImage ouverte par l'utilisateur
	 * @param a   le rayon de l'élément structurant
